...
- **Restaurant Onboarding**: Supports both self-service public registration and invite-based onboarding for restaurant partners, with legal term acceptance tracking. Self-service onboarding fetches active term templates from `term_templates` table and requires individual acceptance of each. Includes optional PNG logo upload (max 2MB) stored via Forge Storage.
