...
- **Quotation Product Selection**: Quotations have a `productId` FK referencing `products` table. The quotation form includes a product selector dropdown. Product name appears in the quotation list under volume. PDF proposal generation uses product-specific labels (unit labels, product name) instead of hardcoded "bolachas de chopp". The `generateQuotationName` function includes product name. Duplicate quotation preserves productId. Simulator defaults to productId=1 (Bolacha de Chopp) until Task #18 adds full multi-product simulator support.
