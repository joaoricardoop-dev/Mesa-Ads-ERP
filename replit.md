...
- **CRM Contacts**: Global contacts management at `/comercial/contatos` (`Contacts.tsx`). Each contact belongs to a client (`contacts` table with clientId FK). Fields: name, email, phone, role, notes, isPrimary. Supports CSV export. Backend: `server/contactRouter.ts` with CRUD + listAll (joins client name).
