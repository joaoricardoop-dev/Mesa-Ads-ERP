...
- **Banco de testes E2E isolado (Task #189)**: O `scripts/pre-deploy.sh` sobe um dev server (NODE_ENV=development) durante o pipeline de deploy para rodar Playwright. Os endpoints `/api/dev-*` fazem INSERT real (parceiros/clientes/campanhas "E2E ..."). Para evitar que esse lixo vaze para produção, o pipeline EXIGE o secret `DATABASE_URL_TEST` (Postgres dedicado, separado do `DATABASE_URL` de produção) e exporta `DATABASE_URL=$DATABASE_URL_TEST` só no subprocess do dev server. Defesa em profundidade: o helper `assertSafeForDevSeed` em `server/_core/index.ts` recusa (503) qualquer call em `/api/dev-*` se `DATABASE_URL_TEST` estiver setado mas `DATABASE_URL` apontar para outro banco. Schema do banco de teste é criado automaticamente por `runMigrations()` na primeira inicialização. Cleanup retroativo do lixo já existente é feito pela migration `inactivate_leaked_e2e_records_task_189` (inativa em vez de DELETE para preservar trilha financeira de invoices já emitidas contra os clientes E2E).
