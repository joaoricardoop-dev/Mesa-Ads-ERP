...
// Task #260 — Condições de pagamento: coerência + edição.
...
//      (GET /api/public-signing/quotation/:token) na fase os_gerada — ambos
//      reconciliados contra a mesma âncora canônica (fonte única).
//   2. A edição das parcelas em os_gerada persiste e reflete imediatamente na
//      tela pública, mantendo as invariantes (nenhum vencimento antes do
//      início do período; Σ parcelas = total).
...
test.describe("condições de pagamento — coerência interna×pública + edição em os_gerada", () => {
  ...
  test("schedule idêntico entre interno e público, e edição em os_gerada reflete na pública", async ({ request }) => {
    ...
    // Invariantes no estado inicial reconciliado.
    for (const it of internal) {
      expect(it.dueDate >= periodStart, `parcela #${it.sequence} (${it.dueDate}) vence antes do início (${periodStart})`).toBeTruthy();
    }
    ...
    const novasParcelas: Parcela[] = [
      { sequence: 1, amount: "1200.00", dueDate: periodStart, notes: null },
    ...
    // Invariantes pós-edição.
    for (const it of pubAfter) {
      expect(it.dueDate >= periodStart).toBeTruthy();
    }
