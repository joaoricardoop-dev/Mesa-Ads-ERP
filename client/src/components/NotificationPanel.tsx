...
interface RestaurantePanelProps {
  open: boolean;
  onClose: () => void;
}

export function RestauranteNotificationPanel({ open, onClose }: RestaurantePanelProps) {
  const queryClient = useQueryClient();
  const [, setLocation] = useLocation();

  const { data: notifications = [], refetch } = trpc.notification.listForRestaurante.useQuery(
    { limit: 50, offset: 0, unreadOnly: false },
    { enabled: open, refetchInterval: open ? 30000 : false }
  );

  const markReadMutation = trpc.notification.markReadForRestaurante.useMutation({
    onSuccess: () => {
      refetch();
      queryClient.invalidateQueries({ queryKey: [["notification", "unreadCountForRestaurante"]] });
    },
  });

  const markAllReadMutation = trpc.notification.markAllReadForRestaurante.useMutation({
    onSuccess: () => {
      refetch();
      queryClient.invalidateQueries({ queryKey: [["notification", "unreadCountForRestaurante"]] });
      toast.success("Todas as notificações foram marcadas como lidas.");
  ...
  const navigateToTab = (n: RestauranteNotificationItem) => {
    if (!n.campaignId) return;
    const shippingEvents = new Set([
      "campaign_status:transito",
      "campaign_status:distribuicao",
    ]);
    const tab = shippingEvents.has(n.eventType) ? "os" : "calendar";
    setLocation(`/portal?tab=${tab}`);
    onClose();
  };
              ...
              {notifications.map((n) => {
                ...
                return (
                  <li
                    ...
                    onClick={n.campaignId ? () => navigateToTab(n) : undefined}
                  >
                    <div className="mt-0.5 shrink-0 text-orange-500">
                      <Megaphone className="w-4 h-4" />
                    </div>
...
export function NotificationBell({ isAdmin, isAnunciante, isRestaurante }: NotificationBellProps) {
  ...
  const { data: restauranteCountData } = trpc.notification.unreadCountForRestaurante.useQuery(undefined, {
    enabled: !!isRestaurante && !isAdmin && !isAnunciante,
    refetchInterval: 30000,
  });
