import { comercialProcedure, protectedProcedure, publicProcedure, router } from "./_core/trpc";
import { z } from "zod";
...
export const quotationRouter = router({
  ...
  generateProposalLink: comercialProcedure
    .input(z.object({ quotationId: z.number() }))
    .mutation(async ({ input }) => {
      const db = await getDatabase();
      const [q] = await db.select().from(quotations).where(eq(quotations.id, input.quotationId));
      if (!q) throw new TRPCError({ code: "NOT_FOUND", message: "Cotação não encontrada." });

      if (q.publicToken) {
        return { token: q.publicToken, url: `/proposta/${q.publicToken}` };
      }
      ...
      return { token, url: `/proposta/${token}` };
    }),

  getPublicProposal: publicProcedure
    .input(z.object({ token: z.string().min(1) }))
    .query(async ({ input }) => {
      const db = await getDatabase();

      const [q] = await db.select().from(quotations).where(eq(quotations.publicToken, input.token));
      if (!q) throw new TRPCError({ code: "NOT_FOUND", message: "Proposta não encontrada." });

      const [client] = await db.select().from(clients).where(eq(clients.id, q.clientId));
      ...
      const restaurants = await db
        ...
        .from(quotationRestaurants)
        .leftJoin(activeRestaurants, eq(quotationRestaurants.restaurantId, activeRestaurants.id))
        .where(eq(quotationRestaurants.quotationId, q.id));
  ...
  signProposal: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (q.signedAt) throw new TRPCError({ code: "BAD_REQUEST", message: "Esta proposta já foi assinada." });

      if (q.status === "perdida" || q.status === "expirada") {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Esta proposta não está mais disponível para assinatura." });
      }

      if (q.validUntil && new Date(q.validUntil) < new Date()) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "O prazo de validade desta proposta expirou." });
      }
