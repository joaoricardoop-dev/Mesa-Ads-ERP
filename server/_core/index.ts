...
async function startServer() {
  ...
  // Endpoints /api/dev-* (fixtures e2e) ficam ISOLADOS em módulo separado
  // (`server/_dev/devEndpoints.ts`) e são carregados via dynamic import APENAS
  // quando a flag `DEV_FIXTURES=1` está armada explicitamente. Como `npm run
  // start` (produção) nunca seta essa flag, o esbuild produz um bundle onde
  // o módulo não é alcançável — mesmo um vazamento de `NODE_ENV=development`
  // em produção não consegue registrar essas rotas. Camadas adicionais (host
  // proibido + sentinel no banco) vivem dentro do módulo.
  if (process.env.DEV_FIXTURES === "1") {
    const { registerDevEndpoints } = await import("../_dev/devEndpoints");
    await registerDevEndpoints(app);
  }
